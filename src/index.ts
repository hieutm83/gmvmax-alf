...
async function consume(message: TaskMessage, env: Env): Promise<void> {
  ...
  if(message.type==='supabase-manual-sync'){await env.DB.prepare("INSERT INTO app_settings(key,value) VALUES('SUPABASE_MANUAL_SYNC_STATUS',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(JSON.stringify({status:'RUNNING',startDate:message.startDate,endDate:message.endDate,progress:10,tables:message.tables})).run();try{const storeId=await resolveDefaultStore(runtime);const input={advertiserId:runtime.DEFAULT_ADVERTISER_ID,storeId,startDate:message.startDate,endDate:message.endDate,forceRefresh:true};const report=await loadMainReport(runtime,input,true);await env.DB.prepare("INSERT INTO app_settings(key,value) VALUES('SUPABASE_MANUAL_SYNC_STATUS',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(JSON.stringify({status:'RUNNING',startDate:message.startDate,endDate:message.endDate,progress:45,tables:message.tables})).run();await loadCreativeSummaries(runtime,{...input,products:report.products||[],allContexts:report.creativeContexts||report.products||[],availableProducts:report.availableProductCount||0,forceRefresh:true}).catch(()=>undefined);await syncSupabaseBackup(env,message.endDate);await env.DB.prepare("INSERT INTO app_settings(key,value) VALUES('SUPABASE_MANUAL_SYNC_STATUS',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(JSON.stringify({status:'SUCCESS',startDate:message.startDate,endDate:message.endDate,progress:100,tables:message.tables})).run();}catch(error){await env.DB.prepare("INSERT INTO app_settings(key,value) VALUES('SUPABASE_MANUAL_SYNC_STATUS',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(JSON.stringify({status:'FAILED',startDate:message.startDate,endDate:message.endDate,progress:100,tables:message.tables,error:String(error)})).run();throw error;}return;}
